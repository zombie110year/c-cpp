...
.. note:: void 类型
   ...
   ``int argc, char *argv[]``, 而是 :c:type:`void`.
...
``const`` 关键字同样可以修饰指针 (:ref:`常量指针与指针常量`) 和函数 (:ref:`常量与函数`).

但实际上, 常量的含义是 "无法通过此标识符修改目标内存",
在一些情况下, 可以通过指针运算定位到常量所表示的内存空间进行修改,
而编译器不会阻止.

但这是 "未定义行为", 不同的编译器有不同的表现,
你可以将以下代码分别用 clang, gcc 或 msvc 编译并运行查看效果:

.. literalinclude:: _code/show.const.modify.c
   :language: c
   :caption: show.const.modify

.. warning:: 这不应当是程序中的正常情况, 这里提出仅仅是告知一种可能导致运行异常的情况.
